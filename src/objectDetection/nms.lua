...
function nms(boxes, overlap, scores)
  ...
  pickedBoxes:resize(boxes:size()):zero()
  local count = 1
  ...
  while I:numel() > 0 do 
    ...
    pick[count] = i
    local newBox = boxes.new():resizeAs(boxes[i])
    count = count + 1
    ...
    -- Perform average of selected bounding boxes
    -- Includes coordinates for picked box
    local G = I[IoU:ge(overlap)]
    xx1:index(x1, 1, G)
    yy1:index(y1, 1, G)
    xx2:index(x2, 1, G)
    yy2:index(y2, 1, G)
  ...
  pick = pick[{{1, count-1}}]
  pickedBoxes = pickedBoxes[{{1, count-1}}]
  return pick, pickedBoxes
end
